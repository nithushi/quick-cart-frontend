...
const ProductItem = ({ id, name, price, image, description, rating = 4 }: ProductProps) => {
  return (
    <Link href={`/product/${id}`} className="group cursor-pointer flex flex-col gap-3 p-7 rounded-xl hover:shadow-xl transition-all duration-300 bg-white border border-transparent hover:border-gray-100">
        ...
        <div className="bg-gray-100 rounded-xl overflow-hidden relative h-60 flex items-center justify-center group-hover:bg-gray-50 transition">
            ...
            {/* Wishlist Heart Icon */}
            <div className="absolute top-3 right-3 bg-white p-2 rounded-full shadow-sm hover:scale-110 transition z-10">
                 <Image src={assets.heart_icon} alt="wishlist" className="w-3 h-3 opacity-70 hover:opacity-100" />
            </div>
